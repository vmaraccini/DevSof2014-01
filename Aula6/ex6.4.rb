...
str.scan(/(\d*:\d*).*\[(.*)\].(.*)/) { |match| puts match[1] + ": " + match[2] + " (" + match[0] + ")" }
...
i = 0
str.scan(/\[economia\](.*dólar.*)/) { |match|; i = match.length }
puts "#{i} notícia(s) fala(m) sobre o dólar."

# 4) Imprimir todos os horários das notícias que contém a palavra golfe.
puts "\n4: Horários das notícias que contém golfe:\n"

str.scan(/(\d*:\d*).*golfe/) { |match|; match.each { |e| puts e } }
...
puts "\n5: \n"
puts str.gsub(/(\[.*\])/) {|tema| tema.upcase}